            ...
            <div class="card-header bg-light">
                <ul class="nav nav-tabs justify-content-center" id="myTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="asset-tab" data-bs-toggle="tab" data-bs-target="#asset" type="button" role="tab" aria-controls="tabs-asset" aria-selected="true"><i class="fa fa-map-marker"></i></a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="legend-tab" data-bs-toggle="tab" data-bs-target="#legend" type="button" role="tab" aria-controls="tabs-profile" aria-selected="false"><i class="fa fa-map-signs"></i></button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="chart-tab" data-bs-toggle="tab" data-bs-target="#tabs-contact" type="button" role="tab" aria-controls="tabs-contact" aria-selected="false"><i class="fas fa-chart-pie"></i></button>
                    </li>
                    <button type="button" class="btn btn-sm pull-right" id="sidebar-hide-btn"><i class="fa fa-chevron-left"></i></button>
                </ul>
            </div>
            <div class="card-body p-1 overflow-x-hidden">
                <h5 class="card-title"><span id="sidebarTitle"></span>
                </h5>
                <div class="tab-content" id="myTabContent">
                    <div class="tab-pane fade show active" id="asset" role="tabpanel" aria-labelledby="tabs-asset-tab">
                        <div class="row overflow-x-hidden">
                            <div class="col-sm-8 col-md-8">
                                <input type="text" class="form-control search" placeholder="Filter Data" />
                            </div>
                            <div class="col-sm-4 col-md-4">
                                <button type="button" class="btn btn-sm btn-primary pull-right sort" data-sort="feature-name" id="sort-btn"><i class="fa fa-sort"></i>Urutkan</button>
                            </div>
                        </div>
                    </div>
                    <div class="tab-pane fade" id="legend" role="tabpanel" aria-labelledby="tabs-profile-tab">Legenda</div>
                    <div class="tab-pane fade" id="tabs-contact" role="tabpanel" aria-labelledby="tabs-contact-tab">Query</div>
                </div>
                {{-- <div class="row">
                        <div class="col-xs-12 col-md-12" id="sidebarContents">
                            <!-- Content Tools -->
                        </div>
                    </div> --}}
            </div>
...
<script>
    let map, featureList, sidebarLayers = [];
    // featureList,
    KelurahanSearch = [];
    // KecamatanSearch = [],
    //   septiktankSearch = [],
    ...
    function kerawananFeature(feature, layer) {
        ...
        let showDataRawan = '<div>' + '<p>' + 'Kel. ' + contentPopup +
            '<br>' + 'Area ' + dataKerawanan + ' Air Limbah' + '</p>' + '</div>'
            , customOptions;
        layer.on({
            click: function(value) {
                layer.bindPopup(showDataRawan).openPopup();
            },
            ...
            mouseover: function(value) {
                ...
                layer.bindTooltip(showDataRawan).openTooltip();
    ...
    let iconLayersControl = new L.Control.IconLayers(
        [{
                ...
                , layer: cartoLight
                , icon: '{{ asset('
                img / basemap / greyscale.png ') }}'
            }
            , {
                title: 'Hybrid'
                , layer: mapboxImagery
                , icon: '{{ asset('
                img / basemap / imagery.png ') }}'
            }
            , {
                title: 'Outdoor'
                , layer: mapboxOutdoor
                , icon: '{{ asset('
                img / basemap / outdoor.png ') }}'
            }
            , {
                title: 'Google Maps'
                , layer: googleMaps
                , icon: '{{ asset('
                img / basemap / googleMaps.png ') }}'
            }
            , {
                title: 'Google Imagery'
                , layer: googleSatellite
                , icon: '{{ asset('
                img / basemap / googleSatellite.png ') }}'
            }
            , {
                title: 'Street Maps'
                , layer: mapboxStreet
                , icon: '{{ asset('
                img / basemap / streetmap.png ') }}'
            }
            , {
                title: 'Topo Map'
                , layer: otopomap
                , icon: '{{ asset('
                img / basemap / topomap.png ') }}'
            }
            , {
                title: 'Dark'
                , layer: mapboxDark
                , icon: '{{ asset('
                img / basemap / dark.png ') }}'
            }
        , ], {
    ...
    // const jalanWMS = L.tileLayer.wms("http://geoservice.kalselprov.go.id:8080/geoserver/DPUPR/wms", {
    ...
    /*load wms form geoserver */
    const sungaiWMS = L.Geoserver.wms("https://www.sialdobjm.com/geoserver/banjarmasin/wms", {
    ...
    const toponimiSungai = L.Geoserver.wms("https://www.sialdobjm.com/geoserver/banjarmasin/wms", {
        layers: 'banjarmasin:Toponimi_Nama_Sungai_PT_50K'
        , format: 'image/png'
        , transparent: true
        , version: '1.1.0'
        , crs: L.CRS.EPSG4326
...
L.marker([<?= $data->lat . ',' . $data->lng ?>], {
    ...
    ";
    $("#feature-title").html(modalTitle); $("#feature-info").html(content); $('#featureModal').modal('show'); highlight.clearLayers().addLayer(L.circleMarker(this.getLatLng(), highlightStyle));
    });
    ...
    septikkomunal = L.geoJson(null, {
        ...
        , onEachFeature: function(feature, layer) {
            if (feature.properties) {
                ...
                $("#feature-list tbody").append('<tr class="feature-row" id="' + L.stamp(layer) + '" lat="' + layer.getLatLng().lat + '" lng="' + layer.getLatLng().lng + '"><td style="vertical-align: middle;"><img width="16" height="18" src="{{ asset('
                    img / septictank.png ') }}"></td><td class="feature-name">' + layer.feature.properties.NAMA_KSM + '</td><td style="vertical-align: middle;"><i class="fa fa-chevron-right pull-right"></i></td></tr>');
    ...
    let overlayTree = {
        ...
        , children: [{
                ...
                , children: [{
                    ...
                    , {
                        label: '&nbsp;NAMA SUNGAI'
                        , layer: toponimiSungai
                    }
                    , {
                        label: '&nbsp;{{ trans('
                        cruds.maps.pipe ') }}'
                        , layer: jaringanAirlimbah
                    },
            ...
            , {
                ...
                , selectAllCheckbox: true
                , children: [{
                        label: "<img src='{{ asset('img/green.png') }}' width='24' height='24'>&nbsp;Tangki Septik Individual"
                        , layer: septiktank
                    }
                    , @foreach($kategori as $data) {
                        label: "<img src='{{ asset( Storage::url($data->icon) ) }}' width='24' height='36'>&nbsp;{{ $data->type }}"
                        , layer: {
                            {
                                $data - > layer
                            }
